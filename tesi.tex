...
\begin{proof}
	...
	{\color{red}\begin{align*}
		...
		\end{align*} 
	The exchange between integral and series is possible since the operator $		$
...
From the assumption that the set of indices is countable we may think to consider a bijective mapping $\sigma : \N \rightarrow \mathcal{J}$ and define the convergence of $\sum_{j \in \mathcal{J}} x_j$ using the map $\sigma$ and the classical definition of convergence of a series indexed by natural numbers. However, since the map $\sigma$ is not unique, the convergence of the series may depend upon the particular choice of the enumeration of $\mathcal{J}$. This is not the case for series that converge unconditionally.
...
\begin{prop}\label{exchange of series and operator}
	Let $A \in \B(X,Y)$. If $\sum_{j \in \mathcal{J}} x_j$ converges unconditionally in $X$ to $x$, then $\sum_{j \in \mathcal{J}} Ax_j$ converges unconditionally to $Ax$.
\end{prop}
\begin{proof}
...
\begin{prop}\label{exchange order double series}
	Suppose that $\sum_{(n,m) \in \N^d} x_{n,m}$ converges unconditionally to $x \in X$. Then the inner partial sum $s_{n,M} = \sum_{m=1}^{M} x_{n,m}$ converges to some $y_n \in X$ for every $n \in \N$ and $x = \sum_{n \in \N} y_n$ with unconditional convergence. Likewise $\sum_{n=1}^{N} x_{n,m}$ converges to some $z_m \in X$ for every $m \in \N$ and $x = \sum_{m \in \N} z_m$.
\end{prop}
\begin{proof}
	...
	\end{equation*}
	for every $N \geq N_0$ and $M \geq M_0$.
\end{proof}

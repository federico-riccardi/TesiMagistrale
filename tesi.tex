...
\begin{proof}
	...
	\end{align}
	When using Cauchy-Schwarz in \eqref{Cauchy Schwarz lemma uncertainty principle}, we have equality if and only if $(A-a)f = \lambda (B-b)f$ for some $\lambda \in \C$, while in \eqref{second inequality lemma uncertainty principle} we have equality if and only if $\langle (A-a)f,(B-b)f\rangle = \lambda \|(B-b)f\|^2$ is purely imaginary, which means that $\lambda = ic$ for some $c \in \R$.
\end{proof}
...
\begin{teo}[Heisenberg's uncertainty principle]\label{Heisenberg's uncertainty principle theorem}
	...
	\end{equation}
	Equality holds if and only if $f$ is a multiple of a Gaussian, i.e. $e^{2 \pi i b \cdot(x-a)}e^{-\pi |x-a|^2/c}$ for some $c > 0$.
\end{teo}
\begin{proof}
	...
	\end{align*}
	and thanks to Plancherel's theorem we have that $\|(\omega_k-b_k)\hat{f}\|_2 = \left\| \left(\frac{1}{2\pi i}\pfrac{}{t_k} - b_k\right)f\right\|_2$, so that now the operator $B$ appearing in \eqref{uncertainty principle for operators formula} is $f \in \mathcal{S}(\R^d) \mapsto Bf = \frac{1}{2\pi i}\pfrac{f}{t_k} \in \mathcal{S}(\R^d)$. Again, this is {\color{red} (formally)} self-adjoint over $\mathcal{S}(\R^d)$, so we can apply Lemma \ref{uncertainty principle for operators lemma}:
	\begin{equation*}
		\left(\int_{\R^d}(t_j-a_j)^2 |f(t)|^2 dt\right)^{1/2} \left(\int_{\R^d} (\omega_k-b_k)^2 |\hat{f}(\omega)|^2 d\omega\right)^{1/2} \geq \dfrac12\lvert\langle [A,B]f,f \rangle \rvert
	\end{equation*}
	Now we just need to compute the commutator of $A$ and $B$. Let $f \in \mathcal{S}(\R^d)$:
	\begin{align*}
		[A,B]f(t) &= BAf(t)-ABf(t) = \dfrac{1}{2\pi i}\pdfrac{}{t_k}(t_jf(t)) - t_j \dfrac{1}{2\pi i}\pdfrac{f}{t_k}(t) =\\
	...
	\end{align*}
	therefore $[A,B] = \frac{1}{2\pi i}\delta_{jk}$, where $\delta_{jk}$ has to be intended as a function identically equal to 1 if $j=k$ or identically equal to 0 if $j \neq k$. Since $A$ and $B$ commute when $j \neq k$ from now on we will only consider the case $j=k$, in which Lemma \ref{uncertainty principle for operators lemma} allows us to conclude that
	...
	\begin{equation}\label{Heisenberg's uncertainty principle differential equation}
		\left(\dfrac{1}{2\pi i}\pdfrac{}{t_k} - b_k\right)f = ic_j (t_j-a_j)f
	\end{equation}
	for some $c_j \in \R$. In principle $c_j$ could be different but equality in \eqref{Heisenberg's uncertainty principle Cauchy Schwarz in Rd} is achieved if and only if $|c_j|=c \geq$, that is all $c_j$ have the same modulus (but possibly different sign). 	PERCHé C NON PUò ESSERE 0???.\\
	We need 
	}
\end{proof}

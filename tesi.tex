...
In this first chapter we briefly recall some basic definition and results on functional analysis and Fourier transform. In section \ref{section basics of functional analysis} basic concepts about operators between Banach and Hilbert spaces are presented. In addition, certain classes of operators, namely the trace class and the Hilbert-Schmidt class, are introduced with some of their most important properties. Then in section \ref{Fourier transform and its properties section} Fourier transform is defined and some of its essential properties are given.
...
The trace of an operator can be defined as it is for matrices. Before proceeding, however, we must define what does it means for an operator to be non-negative.
...
\end{proof}
Before extending the previous lemma to the multi-dimensional case we need the following result related to the Fourier transform of restrictions. Before stating the lemma we introduce the following notation:
\begin{equation*}
	\begin{gathered}
...
\begin{proof}
	...
	\end{itemize}	
\end{proof}
Previous lemma is sufficient in order to prove Heisenberg's uncertainty principle.
...
\end{cor}
Loosely speaking, this result is telling us that $f$ and $\hat{f}$ cannot be simultaneously too concentrated. So, for example, if $f$ is strongly concentrated in the time domain, i.e. $|T|$ is small, the necessarily $|\Omega|$ has to be large. {\color{blue} However, a much stronger result holds.}
\begin{teo}
	Suppose $f \in L^1(\R^d)$, $\mathrm{supp}f \subseteq T$ and $\mathrm{supp}\hat{f} \subseteq \Omega$. If $|T| |\Omega| < +\infty$ then $f=0$.
\end{teo}
{\color{blue}As pointed out in \cite{grochenig}, this results forces us not to talk about ``support of a function'' in the usual way, since in this case the only information we have is that either $f = 0$ or $|\mathrm{supp} f||\mathrm{supp} \hat{f}| = +\infty$. Therefore, a more suitable notion is the one of essential support, since for this we can give a quantitative result, namely \ref{Donoho-Stark's uncertainty principle theorem}.}
...
\begin{prop}\label{monotonicity of maximizer}
	Let $\allowdisplaybreaks[1]\mathcal{C}' = \{u : (0,+\infty) \rightarrow [0,+\infty) \text{ such that } u \text{ is measurable and satisfies } \eqref{constraints generic case distribution function}$. Then
	\begin{equation}

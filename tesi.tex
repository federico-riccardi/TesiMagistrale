...
\end{proof}
Before extending the previous lemma to the multi-dimensional case we need the following result.
\begin{lem}\label{Fourier transform for restrictions}
	Let $ f \in L^2(\R^d)$. We let $t' = (t_2, \ldots, t_d) \in \R^{d-1}$, $\omega' = (\omega_2, \ldots, \omega_d) \in \R^{d-1}$ and define $\F_{t_1} f (\omega_1) = \F(f(\cdot, t'))(\omega_1)$, $\F_{t'} f(\omega') = \F (f(t_1, \cdot))(\omega')$. Then $\F_{t_1} f (\omega_1) \in L^2(\R^{d-1})$ for almost every $\omega_1 \in \R$ and $\F_{t'} (\F_{t_1} f (\omega_1))(\omega') = \F f(\omega)$.
\end{lem}
\begin{proof}
	Before starting, we point out that, since $f \in L^2(\R^d)$, $f(\cdot, t') \in L^2(\R)$ for almost every $t' \in \R^{d-1}$, therefore $\F_{t_1} f(\omega_1)$ is well-defined for almost every $t' \in \R^{d-1}$. Then we have:
	\begin{align*}
		\int_{\R^d} |\F_{t_1} f (\omega_1)|^2 d\omega_1 dt' \overset{\mathrm{Tonelli}}&{=} \int_{\R^{d-1}} \int_{\R} |\F(f(\cdot,t'))(\omega_1)|^2 d\omega_1 dt' \overset{\mathrm{Plancherel}}{=}\\
																					  &= \int_{\R^{d-1}} \int_{\R} |f(t_1,t')|^2 dt_1 dt' = \|f\|_2.
	\end{align*}
	which proves that $\F_{t_1} f(\omega_1)$ is in $L^2(\R^{d-1})$ for almost every $\omega_1 \in \R$.
	
	Now suppose that $f \in L^1(\R^d) \cap L^2(\R^d)$. Since $f$ is in $L^1(\R^d)$ we can use \eqref{Fourier transform formula} to express $\F_{t_1} f$:
	\begin{equation*}
	\begin{gathered}
		\F_{t_1} f(\omega_1) = \int_{\R} f(t_1,t') e^{-2 \pi i \omega_1 t_1} dt_1 \implies \\
		\F_{t'} (\F_{t_1} f(\omega_1))(\omega') = \int_{\R^{d-1}} \left(\int_{\R} f(t_1,t') e^{-2 \pi i \omega_1 t_1} dt_1\right) e^{-2 \pi i \omega' \cdot t'} dt' \overset{\mathrm{Fubini}}{=}\\
...
\end{lem}
\begin{proof}
	Without loss of generality we consider the case $j=1$. For the ease of notation let $t' = (t_2, \ldots, t_d) \in \R^{d-1}$ and $dt' = dt_2 \cdots dt_d$ ({\color{blue} and the same for $\omega$}). Moreover, we introduce the following notation:
	\begin{equation*}
		\langle f, g \rangle_{t_1} = \langle f(\cdot, t'), g(\cdot,t') \rangle \quad \langle f, g \rangle_{t'} = \langle f(t_1, \cdot), g(t_1, \cdot) \rangle
	\end{equation*}
	
	From Lemma \ref{Heisenberg's uncertainty principle lemma}, since $f (\cdot, t') \in L^2(\R)$ for a.e. $t' \in \R^{d-1}$, we have:
	\begin{equation*}
		\int_{\R} t_1^2 |f(t_1, t')|^2 dt_1 + \int_{\R} \omega_1^2 |\F_{t_1} f(\omega_1)|^2 d\omega_1 = \dfrac{1}{2\pi} \sum_{k_1=0}^{+\infty} (2k_1 + 1) |\langle f, H_{k_1} \rangle_{t_1}|^2
	\end{equation*}
	...
	\begin{itemize}
		...
		\item $\displaystyle \int_{\R^{d-1}} \int_{\R}  \omega_1^2|\F_{t_1} f(\omega_1)|^2 d\omega_1 dt' \overset{\mathrm{Tonelli}}{=} \int_{\R} \omega_1^2 \int_{\R^{d-1}} |\F_{t_1} f(\omega_1)|^2 dt' d\omega_1$.\vspace{0.1mm}\\
		
		From Lemma \ref{Fourier transform for restrictions} we know that $\F_{t_1} f(\omega_1)(\omega_1) \in L^2(\R^{d-1})$ for a.e. $\omega_1 \in \R$. Therefore, we can use Plancherel's theorem in the inner integral for a.e. $\omega_1 \in \R$ and obtain:
		\begin{align*}
			\int_{\R^{d-1}} \int_{\R}  \omega_1^2 |\F_{t_1} f(\omega_1)|^2 d\omega_1 dt' &= \int_{\R} \omega_1^2\int_{\R^{d-1}} |\F_{t'} (\F_{t_1} f(\omega_1))(\omega')|^2 d\omega' d\omega_1 \overset{\ref{Fourier transform for restrictions}}{=}\\
